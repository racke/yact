...
sub init_conference {
    ...
    Git::Repository->run( init => '--bare', $fake_remote, \%o );
    Git::Repository->run( clone => "file://$fake_remote", $temp->name, \%o );
    my $r = Git::Repository->new( work_tree => $temp->name, \%o );

    Config::INI::Writer->write_file( $conf_config{$conf_name},
        file( $temp->name, 'yact.ini' ) );

    $r->run( add    => '.' );
    $r->run( commit => -m => 'Generated by YACTTest::Conferences' );
